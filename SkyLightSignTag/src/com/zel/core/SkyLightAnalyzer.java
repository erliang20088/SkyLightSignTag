...
public class SkyLightAnalyzer {
	...
	public String getSplitResult(String beginTag,String endTag) {
		StringBuilder sb=new StringBuilder();//先把原始串保存进sb中
...
//				continue;
				...
				case 1:// 直接加入结果集
					...
					if (isValidWord) {// 后边有字符且该temp_string是全字母,且后边一个是字母的话就不添加了
						sb.append(beginTag+this.content.substring(begin, current + 1)+endTag);//追加进sb集合中
						found_new=true;
					}
					endFor = true;
				...
				case 2:// 继续往下循找
						...
						if (temp_position >= 0) {// 说明找到下一个char
							...
							case 0:
								if (isFound) {
									...
									if (isValidWord) {
										sb.append(beginTag+this.content.substring(begin, found_pos + 1)+endTag);//追加进sb集合中
										found_new=true;
										begin = found_pos;
									}
								}
					...
					if (current + 1 == array_length) {// 说明此时已经到头了直接判断这个是不是词就可以了!
						...
						case 2:
							...
							if (isValidWord) {
								sb.append(beginTag+this.content.substring(begin, array_length)+endTag);//追加进sb集合中
								found_new=true;
								begin = current;
							}
							break;
			...
			isFound = false;
			if(!found_new){
				sb.append(this.content.substring(begin,(begin+1<=array_length?begin+1:array_length)));				
			}
		}
		return sb.toString();
	}
